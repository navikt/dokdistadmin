...
import static no.nav.dokdistadmin.administrerforsendelse.filinfo.FilInfoMapper.VALID_FILTYPER;
...
public class FilInfoValidator {
	...
	private static void validerFilnavnOgTypeHvisFilInfoIdErSatt(FilInfoRequest request) {
		if (erFilnavnEllerTypeNotNull(request)) {
	...
	private static void validerFilnavnOgTypeHvisFilInfoIdErNull(FilInfoRequest filInfoRequest) {
		if (erFilnavnEllerTypeNull(filInfoRequest)) {
	...
	private static void validerFilType(FilInfoRequest filInfoRequest) {
		if (isNotBlank(filInfoRequest.filtype()) && !VALID_FILTYPER.contains(filInfoRequest.filtype())) {
			throw new UgyldigInputException(format("Ugyldig input: %s er ikke en gyldig kodeverdi i %s", filInfoRequest.filtype(), VALID_FILTYPER));
		}
	}
